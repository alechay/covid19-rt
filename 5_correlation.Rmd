...
get_regressions <- function(data, lag) {
  if(lag==0) {
    retail <- lm(Rt ~ retail + date - 1, data = data)
    grocery <- lm(Rt ~ grocery + date - 1, data = data)
    parks <- lm(Rt ~ parks + date - 1, data = data)
    transit <- lm(Rt ~ transit + date - 1, data = data)
    workplaces <- lm(Rt ~ workplaces + date - 1, data = data)
    residential <- lm(Rt ~ residential + date - 1, data = data)
    global <- lm(Rt ~ global + date - 1, data = data)
  } else if(lag==7) {
    retail <- lm(Rt ~ retail_lag7 + date - 1, data = data)
    grocery <- lm(Rt ~ grocery_lag7 + date - 1, data = data)
    parks <- lm(Rt ~ parks_lag7 + date - 1, data = data)
    transit <- lm(Rt ~ transit_lag7 + date - 1, data = data)
    workplaces <- lm(Rt ~ workplaces_lag7 + date - 1, data = data)
    residential <- lm(Rt ~ residential_lag7 + date - 1, data = data)
    global <- lm(Rt ~ global_lag7 + date - 1, data = data)
  } else if(lag==14) {
    retail <- lm(Rt ~ retail_lag14 + date - 1, data = data)
    grocery <- lm(Rt ~ grocery_lag14 + date - 1, data = data)
    parks <- lm(Rt ~ parks_lag14 + date - 1, data = data)
    transit <- lm(Rt ~ transit_lag14 + date - 1, data = data)
    workplaces <- lm(Rt ~ workplaces_lag14 + date - 1, data = data)
    residential <- lm(Rt ~ residential_lag14 + date - 1, data = data)
    global <- lm(Rt ~ global_lag14 + date - 1, data = data)
  } else {
    stop('Must enter 0, 7, or 14')
  }
  return(list(Retail=retail, Grocery=grocery, Parks=parks, Transit=transit, Workplaces=workplaces, Residential=residential, Global=global))
