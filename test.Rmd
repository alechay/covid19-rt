...
get_correlations <- function(data, lag) {
  if(lag==0) {
    retail <- cor.test(data$Rt, data$retail, method = 'pearson')
    grocery <- cor.test(data$Rt, data$grocery, method = 'pearson')
    parks <- cor.test(data$Rt, data$parks, method = 'pearson')
    transit <- cor.test(data$Rt, data$transit, method = 'pearson')
    workplaces <- cor.test(data$Rt, data$workplaces, method = 'pearson')
    residential <- cor.test(data$Rt, data$residential, method = 'pearson')
    global <- cor.test(data$Rt, data$global, method = 'pearson')
  } else if(lag==7) {
    retail <- cor.test(data$Rt, data$retail_lag7, method = 'pearson')
    grocery <- cor.test(data$Rt, data$grocery_lag7, method = 'pearson')
    parks <- cor.test(data$Rt, data$parks_lag7, method = 'pearson')
    transit <- cor.test(data$Rt, data$transit_lag7, method = 'pearson')
    workplaces <- cor.test(data$Rt, data$workplaces_lag7, method = 'pearson')
    residential <- cor.test(data$Rt, data$residential_lag7, method = 'pearson')
    global <- cor.test(data$Rt, data$global_lag7, method = 'pearson')
  } else if(lag==14) {
    retail <- cor.test(data$Rt, data$retail_lag14, method = 'pearson')
    grocery <- cor.test(data$Rt, data$grocery_lag14, method = 'pearson')
    parks <- cor.test(data$Rt, data$parks_lag14, method = 'pearson')
    transit <- cor.test(data$Rt, data$transit_lag14, method = 'pearson')
    workplaces <- cor.test(data$Rt, data$workplaces_lag14, method = 'pearson')
    residential <- cor.test(data$Rt, data$residential_lag14, method = 'pearson')
    global <- cor.test(data$Rt, data$global_lag14, method = 'pearson')
  } else {
...
get_regressions <- function(data, lag) {
  if(lag==0) {
    retail <- lm(Rt ~ retail, data = data)
    grocery <- lm(Rt ~ grocery, data = data)
    parks <- lm(Rt ~ parks, data = data)
    transit <- lm(Rt ~ transit, data = data)
    workplaces <- lm(Rt ~ workplaces, data = data)
    residential <- lm(Rt ~ residential, data = data)
    global <- lm(Rt ~ global, data = data)
  } else if(lag==7) {
    retail <- lm(Rt ~ retail_lag7, data = data)
    grocery <- lm(Rt ~ grocery_lag7, data = data)
    parks <- lm(Rt ~ parks_lag7, data = data)
    transit <- lm(Rt ~ transit_lag7, data = data)
    workplaces <- lm(Rt ~ workplaces_lag7, data = data)
    residential <- lm(Rt ~ residential_lag7, data = data)
    global <- lm(Rt ~ global_lag7, data = data)
  } else if(lag==14) {
    retail <- lm(Rt ~ retail_lag14, data = data)
    grocery <- lm(Rt ~ grocery_lag14, data = data)
    parks <- lm(Rt ~ parks_lag14, data = data)
    transit <- lm(Rt ~ transit_lag14, data = data)
    workplaces <- lm(Rt ~ workplaces_lag14, data = data)
    residential <- lm(Rt ~ residential_lag14, data = data)
    global <- lm(Rt ~ global_lag14, data = data)
  } else {
    stop('Must enter 0, 7, or 14')
  }
  return(list(Retail=retail, Grocery=grocery, Parks=parks, Transit=transit, Workplaces=workplaces, Residential=residential, Global=global))
...
```{r}
# Wave 1
d <- get_data(d_mob, d_rt, '2020-03-17', '2020-05-19', COLS)
p <- get_data(p_mob, p_rt, '2020-03-17', '2020-05-19', COLS)
t <- get_data(t_mob, t_rt, '2020-03-17', '2020-05-19', COLS)
h <- get_data(h_mob, h_rt, '2020-03-17', '2020-05-19', COLS)
